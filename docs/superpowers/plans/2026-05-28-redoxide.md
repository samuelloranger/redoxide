...
> `$DEPLOY_DIR` below is the host-side directory holding the Minecraft docker-compose stack. It lives outside this repository; set it to wherever your stack is deployed.

**Files:**
- Modify: `$DEPLOY_DIR/docker-compose.yml`
- Create: `$DEPLOY_DIR/config.toml`

- [ ] **Step 1: Stop Infrared**

```bash
cd $DEPLOY_DIR && docker-compose stop infrared && docker-compose rm -f infrared
```
